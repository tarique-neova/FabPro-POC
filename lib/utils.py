...
def sendkeys_element_without_click(driver, locator, data):
    try:
        wait_for_element_to_be_present_using_xpath(driver, locator)
...
def sendkeys_element_by_locator(driver, locator, data):
    try:
        wait_for_element_to_be_present_using_xpath(driver, locator)
        btn = driver.find_element_by_xpath(locator)
        btn.click()
...
def capture_screenshot(driver, file_name):
    try:
        ...
        logging.error("Unable to Store Screenshot - Exception => " + str(ex))
        pass
