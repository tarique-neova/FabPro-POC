...
from locators import login,homepage,job
...
def create_new_job(driver, casedata):
    ...
    logging.info("casedata jobname{}".format(casedata['create_job_data']['job_name']))
    fill_data_job_info(driver, casedata['create_job_data'])
    # case_data = conftest.parse_test_cases_testdata('job_details')
    # logging.info("casedata jobname{}".format(case_data['create_job_data']['job_name']))
    click_next_button(driver)
    fill_cost_code(driver,casedata['create_job_data'])
    click_next_button(driver)
    fill_package_info(driver,casedata['create_job_data'])
    click_next_button(driver)
    fill_drawing_info(driver,casedata['create_job_data'])
    click_next_button(driver)
    fill_item_info(driver,casedata['create_job_data'])
    pass
...
def fill_package_info(driver, casedata):
    ...
    select1.select_by_visible_text('RL Test Flow Creation')
    time.sleep(3)
    # due date
    due_date_field = utils.get_element(driver, job.DUE_DATE)
...
def fill_drawing_info(driver, casedata):
...
def fill_item_info(driver, casedata):
...
def verify_job_info_update(driver, casedata):
    # TODO:
     # get text of current data and verify it on UI
...
def search_existing_job(driver,casedata):
    ...
    utils.click_element_by_locator(driver, job.JOB_NAME_GET_FIRST_ROW)
